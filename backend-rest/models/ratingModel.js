...
const deleteCommentRatingById = async (params, next) => {
    try {
        const [rows] = await promisePoolRegUser.execute(`DELETE FROM CommentRating WHERE Commentid = "${params}";
                        `);
        return rows;
    } catch (e) {
...
const deleteRecipeRatingById = async (params, next) => {
    try {
        const [rows] = await promisePoolRegUser.execute(`DELETE FROM RecipeRating WHERE Recipeid = "${params}";
                        `);
        return rows;
    } catch (e) {

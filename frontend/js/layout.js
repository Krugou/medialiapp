...
function frontPage() {
    pageGeneration.innerHTML = ` <section id="frontUp" class="frontUp"> <div class="recipeSearch"> <input type="text" id="typeInputField" aria-label="Reseptien haku" placeholder="Hae Resepti" class="Haeresepti marginhalfrem"> <button id="filter" class="filterStyle marpad1rem">Filter</button><button  id="hae" class="filterStyle marpad1rem">Hae</button></div> <div id="filterModal" class="modal"> <div class="modal-content"> <span class="close">&times;</span> <div class="modaladd"> <h3>Add Filters</h3> <div class="filters"> </div><div class="filtersave"> </div></div></div></div><div class="addRecipes"> <button href="addRecipe.html" id="addrecipesButton" class="marpad1rem">Tee resepti</a></button> </div></section> <h2 id="frontPageHeader" class="otsikko"> Uusimmat Reseptit</h2> <hr> <div class="reseptit" id="presentationdata"> </div>`
}
function clearPage() {
...
async function oldestPresentationData() {
    ...
    for (let i = 0; i < (json.length); i++) {
        ...
        if (json[0].Imagefilepath === 'null') {
            ...
            loadout += '<figure class="recipefigure"><img src="' + replaceimage + '"><p>' + json[0].Recipename + '</p><p>' + json[0].Recipetime + '</p><p>' + json[0].Coursetype + '</p><p>' + json[0].Mealtype + '</p> <button > Katso resepti</button ></figure >'
        } else {
            loadout += '<figure class="recipefigure"><img src="' + url + '/' + json[0].Imagefilepath + '"><p>' + json[0].Recipename + '</p><p>' + json[0].Recipetime + '</p><p>' + json[0].Coursetype + '</p><p>' + json[0].Mealtype + '</p> <button > Katso resepti</button ></figure >'
        }

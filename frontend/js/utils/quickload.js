...
function wholePage() {
    ...
    document.body.innerHTML = '<img src="https://www.metropolia.fi/sites/default/files/styles/logo_logobank/public/logos/thumbnails/metropolia_l_black_negative.png" alt="metropolia" > <br> <h1>Okay lets try the metropolia educloud server first</h1> '
        ...
        fetch('https://10.114.34.72/jak/status')
            .then(response => {
            ...
            .catch(error => {
                fetch('http://localhost:3000/status')
                    .then(response => {
                        ...
                        let body = getElementsById('link');
                        body.innerHTML = '<p>Not redirected? Go to <a href="http://localhost:3000/status">status</a></p>';
                    })
                    .catch(error => {
                        ...
                        img.setAttribute('src', 'https://place-puppy.com/' + parseInt(screen.width * 0.25) + 'x' + parseInt(screen.height * 0.25));
